...
def find_matching_owa_alerts(wag_zone_geometry, future_days=14):
    ...
    try:
        ...
        if wag_zone_geometry['type'] not in ['Polygon', 'MultiPolygon', 'Point', 'LineString']:
            celery_logger.info(f"Invalid geometry type in find_matching_owa_alerts: {wag_zone_geometry['type']}")
        ...
        celery_logger.info(f"Error validating or querying OWA alerts: {e}")
        return []
    ...
    try:
        ...
        if shape_obj.geom_type == 'Polygon':
           ...
           return centroid.y, centroid.x # Returns a tuple of latitude and longitude.
            ...
            celery_logger.info(f"Invalid GeoJSON type. Type: {shape_obj.geom_type}")
            return None, None
    except Exception as e:
...
def send_alert_notification_zone_creation_email(email_data, mail_server, mail_port, mail_username, mail_password):
    try:
        ...
        with MongoClient(MONGODB_URI, tlsCAFile=certifi.where()) as client:
            ...
            if success:
                celery_logger.info(f"Alert zone creation email sent to  for alert ID: {alert_id}")
            else:
...
def create_map_image_url(center_lat, center_lon, geojson_str, api_key, color):
    ...
    try:
        ...
        if geojson_data and geojson_data.get("type") == "Polygon":
            ...
            celery_logger.info(f"Map URL generated: {map_url}")
            return map_url
...
def send_weather_alert(user_email, owa_alert, wag_alert_id):
    ...
    try:
        ...
        if geometry:
            celery_logger.info(f"Geometry found: ")  # Check what the geometry is
            try:
                geojson.loads(json.dumps(geometry))
                celery_logger.info("Geometry is valid")
            except Exception as e:
                celery_logger.info(f"Error validating geometry: {e}")
        else:
            celery_logger.info("No geometry found")
...
def check_for_and_send_alerts():
    ...
    celery_logger.info("check_for_and_send_alerts task is running")  # Added this log

    try:
        celery_logger.info("Starting check_for_and_send_alerts process")
        with MongoClient(MONGODB_URI, tlsCAFile=certifi.where()) as client:
            db = client[WAG_DATABASE_NAME]
            ...
            celery_logger.info(f"Number of users found: {len(list(users))}")

            for user in users:
                user_id = user["_id"]
                user_email = user["email"]
                wag_zone_geometry = user.get("geometry", None)
                celery_logger.info(f"Processing user: {user_id}, User ID: {user_id}, Geometry: {wag_zone_geometry}")
                celery_logger.info(f"USER: {user}") # Added this log
                if user.get('email_alerts_enabled', False) == True:  # Check if email alerts are enabled before processing.
                    celery_logger.info(f"Email alerts enabled for user: {user_id}")
                    if wag_zone_geometry:
                        celery_logger.info(f"Finding matching alerts for user: {user_id}")
                        ...
                        celery_logger.info(f"Number of matching alerts for user: {user_id}: {num_alerts}")
                        if num_alerts > 0:
                            # Log a sample of the alerts, not all of them
                            sample_alerts = new_alerts[:min(num_alerts, 5)]  # Limit to 5 for logging
                            celery_logger.info(f"Sample of matching alerts for user: {user_id}: {sample_alerts}")
                            for owa_alert in new_alerts:
                                celery_logger.info(
                                    f"Sending alert for User: {user_id}, Alert ID: {user['_id']}, OWA Alert: {owa_alert}")
                            ...
                            celery_logger.info(f"No matching alerts found for user: {user_id}")
                    else:
                        celery_logger.info(f"User: {user_id} has no valid geometry defined")
                else:
                    celery_logger.info(f"Email alerts are disabled for user: {user_id} - Skipping")
            celery_logger.info("Completed check_for_and_send_alerts process")
    except Exception as e:  # Colon added here
        celery_logger.exception("Error in check_for_and_send_alerts:")
